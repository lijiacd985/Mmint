...
def run(parser):
    ...
    filename = args.peakfile
    if filename.rfind('/')>=0:
        filename = filename[filename.rfind('/')+1:]
    with open (filename + "meth.bed","w") as f:
    ...
    try:
            ...
            #if filename.rfind("/")>=0:
            #    filename = filename[filename.rfind('/')+1:]
            formatfile2=open(filename + "meth.bed",'r')
#Original version always store the file where the peakfile is.
#This version will store the file where people run the script
    except IOError:
            print >>sys.stderr, 'cannot open', filename
            raise SystemExit
